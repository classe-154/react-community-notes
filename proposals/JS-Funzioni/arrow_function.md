...
Immagina il `this` come il riferimento alla "casa" (l'oggetto) dove risiedi:

- **Funzione Tradizionale:** È come entrare in una stanza chiusa a chiave. Appena entri, perdi il riferimento all'esterno. Il `this` cambia e non sai più chi è il proprietario di casa.
...
const persona = {
  ...
  salutaDopoUnSecondo: function() {
    // 1. FUNZIONE TRADIZIONALE (La stanza chiusa)
    setTimeout(function() {
    ...
    setTimeout(() => {
      console.log("Ciao, sono " + this.nome);
    }, 1000); // Output: "Ciao, sono Mario" (Ha guardato fuori e ha trovato il 'nome')
  }
};
