...
5. [Esempio pratico combinato](#-5-esempio-pratico-combinato)
...
- `.`**`forEach(callbackFunction)`**: Sostituisce il ciclo for. Passa in rassegna tutti gli elementi uno per uno ed esegue la callbackFunction su ogni elemento, ma non restituisce un nuovo array.
    
- `.`**`map(callbackFunction)`**: Prende un array, trasforma ogni singolo elemento secondo le istruzioni della callbackFunction e restituisce un nuovo array modificato, con la stessa lunghezza di quello di partenza. (Nota Bene: Per ottenere l'array con dentro i nuovi valori modificati, la callbackFunction deve restituire questi valori modificati tramite `return`. Altrimenti otterrai un array di `undefined` che è il **return standard delle funzioni in JavaScript**)
    
- `.`**`filter(callbackFunction)`**: Scorre l'array e trattiene solo gli elementi che superano un controllo logico specificato dalla callbackFunction (che deve restituire `true` o `false` in base a se l'elemento passa la nostra condizione di filtraggio o meno). Restituisce un nuovo array più corto o vuoto. (Nota Bene: se ti dimentichi di fare `return` nella callback, la callback restituirà undefined che è un valore `falsy` quindi verranno esclusi tutti i valori dell'array iniziale e l'array restituito sarà vuoto) 
...
const prezziRaddoppiati = prezziEuro.map((prezzo) => prezzo * 2); // Nota il return implicito della arrow function, questa restituisce prezzo * 2
// prezziRaddoppiati: [20, 40, 60, 80]

// 3. .filter() per tenere solo i prezzi maiores di 25
const prezziAlti = prezziEuro.filter((prezzo) => prezzo > 25); // Nota il return implicito della arrow function, questa restituisce true o false a seconda di se prezzo è > 25 o meno
// prezziAlti: [30, 40]
```

## 🛒 5. **Esempio pratico combinato**
