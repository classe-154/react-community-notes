...
La `POST` viola questo principio. Questo è il motivo per cui un doppio clic involontario sul pulsante "Acquista" non viene visto dal server come una ripetizione innocua, ma come un'azione del tutto nuova, rischiando così di causare un doppio ordine o un doppio addebito.
    
*** Il "Fix" per i doppi clic sui siti web**
Visto che la POST non è idempotente, gli sviluppatori usano un trucco chiamato Idempotency Key (Chiave di idempotenza) per i pagamenti.
